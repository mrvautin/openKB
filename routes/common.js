...
exports.validate_permalink = function(db, data, callback) {
    ...
    if (typeof data.kb_permalink === 'undefined' || data.kb_permalink === '') {
        ...
        db.kb.count({
            'kb_permalink': data.kb_permalink
        }, function(err, kb) {
            if (kb > 0) {
...
exports.config_expose = function(app) {
    var config = exports.read_config();
    var clientConfig = {};
    clientConfig.route_name = config.settings.route_name !== undefined
        ? config.settings.route_name
        : 'kb';
    clientConfig.add_header_anchors = config.settings.add_header_anchors !== undefined
        ? config.settings.add_header_anchors
        : false;
    clientConfig.links_blank_page = config.settings.links_blank_page !== undefined
        ? config.settings.links_blank_page
        : true;
    clientConfig.typeahead_search = config.settings.typeahead_search !== undefined
        ? config.settings.typeahead_search
        : true;
    clientConfig.enable_spellchecker = config.settings.enable_spellchecker !== undefined
...
exports.setTemplateDir = function(type, req) {
    ...
    if (type !== 'admin') {
        ...
        var layoutDir = config.settings.theme
        ...
        var viewDir = config.settings.theme
            ? path.join(__dirname, '../public/themes/', config.settings.theme, '/views')
