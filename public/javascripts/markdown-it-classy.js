...
function isValidClassChar(code) {
    return (code >= CHAR_0 && code <= CHAR_9) || (code >= CHAR_A_UPPER && code <= CHAR_Z_UPPER) || (code >= CHAR_A_LOWER && code <= CHAR_Z_LOWER) || code === CHAR_UNDERSCORE || code === CHAR_DASH || code === CHAR_SPACE;
}
...
function parse(state) {
    ...
    for (i = state.tokens.length - 1; i >= 0; i -= 1) {
        if (state.tokens[i].type === "em_close" || state.tokens[i].type === "strong_close") {
            break;
        }

        if (state.tokens[i].type === "em_open" || state.tokens[i].type === "strong_open") {
...
function getClassyFromInlineToken(inlineToken) {
    ...
    if (tokens[numChildren - 1]) {
        if (tokens[numChildren - 1].type !== "classy" || tokens.length === 1) {
            return null;
        }
    }
...
function getOpeningToken(tokens, preferOuter, currentIndex) {
    ...
    if (preferOuter && tokens[closingTokenIndex + 1] && /_close$/.test(tokens[closingTokenIndex + 1].type)) {
        closingTokenIndex += 1;
    }
    ...
    for (i = currentIndex; i >= 0; i -= 1) {
        if (tokens[i].type === openingTokenType && tokens[i].level === tokens[closingTokenIndex].level) {
            return tokens[i];
        }
    }
}
